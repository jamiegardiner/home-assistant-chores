...
3. Run `make format && make check` after changes. This covers Python formatting/linting (ruff), type checking (mypy), tests (pytest), and markdown formatting across the whole repo (mdformat) — fix any failures before proceeding. Also check IDE diagnostics using `mcp__ide__getDiagnostics` to catch inspection warnings that ruff and mypy don't surface — fix all warnings before committing.
