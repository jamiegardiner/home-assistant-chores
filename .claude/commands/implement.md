...
1. Run `make format && make check` after changes. This covers Python formatting/linting (ruff), type checking (mypy), tests (pytest), and markdown formatting across the whole repo (mdformat) — fix any failures before proceeding.
1. Commit the code changes following [Conventional Commits](https://www.conventionalcommits.org/) style: `type(scope): description`. The scope is always `issue-<number>`. Map the category label to the commit type:
...
1. After the code commit, review whether `README.md` or `CLAUDE.md` need updating to reflect the change (e.g. new services, changed behaviour, new configuration options, updated architecture). If so, update them and commit separately with the message `docs(issue-<number>): update documentation`.
