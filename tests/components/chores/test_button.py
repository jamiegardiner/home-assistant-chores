...
class TestButtonDeviceInfo:
    ...
    def test_device_info_matches_across_buttons(self):
        """All three buttons share the same device identifiers."""
    ...
    def test_device_info_name_updates_with_coordinator_data(self):
        coordinator = FakeCoordinator()
        button = _make_complete_button(coordinator=coordinator)
        coordinator.data = {**CHORE_STATE, "name": "Bins"}
        assert button.device_info["name"] == "Bins"
...
class TestUniqueIdsDistinct:
    ...
    def test_unique_ids_differ_across_entries(self):
        b1 = _make_complete_button(entry=_make_entry(entry_id="entry_1"))
        b2 = _make_complete_button(entry=_make_entry(entry_id="entry_2"))
